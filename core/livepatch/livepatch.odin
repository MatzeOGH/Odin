...
// Section the compiler emits every fresh-per-reload constant into under -livepatch:
...
// One entry of the compiler-emitted `__odin_livepatch_new_global_inits` table: a
...
// `runtime.type_table` is swapped, see `lp_advance_live_types`). It is exactly what
...
// One entry of the compiler-emitted `__odin_livepatch_{pre,post}_patch_hooks` table
...
// Layout of `__odin_livepatch_{pre,post}_patch_hooks`: a count followed by that many
// name entries.
...
// finds it by dereferencing `__odin_livepatch_type_table_ref`, but resolves this directly as
...
// the LAST reload rather than app start (see `lp_advance_live_types`). The map + its keys
...
lp_live_types_by_name :: proc() -> map[string]^runtime.Type_Info {
	...
	old_tis := lp_read_type_infos(lp_resolve_pdb(LP_TYPE_INFOS_SYM))
...
// the swap pins alive (via `_lp_owner[tt_ref]`) — never a stale/older generation. Rebuilt
...
// code) sees the edited/new types. Unlike `lp_build_type_changes` (which needs a live
...
// One entry of the compiler-emitted `__odin_livepatch_func_hashes` table: a procedure's
// change-detection hashes. `name_hash` is FNV-1a-64 of the link name (see `lp_fnv64`);
...
// must match the C++ emitter (`lb_livepatch_emit_func_hashes`) in src/llvm_backend.cpp.
Func_Hash :: struct {
	name_hash:    u64,
	content_hash: u64,
}
...
// once from the exe's baseline `__odin_livepatch_func_hashes` and updated after each
...
// Read a `__odin_livepatch_func_hashes` table (layout { i64 count; Func_Hash[count] })
...
// re-entrant or thread-safe: they mutate process-lifetime state (`_lp_syms`, `_lp_cur`,
...
Lp_Generation :: struct {
	serial: int,                            // identity; also stored in `_lp_owner` for live-reference tracking
	...
	ranges: [dynamic]Lp_Range,              // membership test for the stack walk (blocks + near pages)
	owned:  [dynamic]uintptr,               // exe addresses (patched proc entries + refreshed globals) that reference this gen's blocks
}
// Process-lifetime; pinned to the OS heap (caller-independent), like `_lp_syms`.
...
// Reload `obj_path` into this running process, patching every `@(livepatch)`
// procedure to its fresh implementation. The set of procedures to patch is
// discovered structurally from the running exe (see `lp_is_hot_entry`), so the
// caller does not have to list them. Returns true if every hot procedure was
// patched. The exe must have been built with `-livepatch -debug` (a PDB next to
// the exe), which the loader uses to resolve the running exe's symbols.
//
// Call from a SINGLE thread, one reload at a time (not re-entrant — see `_lp_busy`).
...
// `-build-mode:obj -livepatch` build emits one object per (non-empty) package, so the
...
//
//     odin build <pkg_dir> -livepatch-patch
//
// where `<pkg_dir>` is the base package directory the `-livepatch` exe build recorded in the
...
//
//     if livepatch.apply_patch() { /* new code is live */ }
//
// `odin` defaults to "odin" on PATH — pass an explicit path if it is not there. `manifest`
// defaults to "odin-livepatch.manifest" in the working directory (where the base build writes
...
// Load a SET of COFF objects produced by a separate-modules `-build-mode:obj -livepatch`
...
apply_many :: proc(obj_paths: []string) -> bool {
	...
	// `lp_resolve_pdb`, which caches). `tls_cache` memoizes each thread-local's exe
	...
	if !_lp_cur_ready {
		_lp_cur = make(map[u64]u64, runtime.heap_allocator()) // process-lifetime: pin to the OS heap
	...
	// `__odin_livepatch_build_id` into the exe and every object built against it; mismatch
	...
	for &o, oi in objs {
		...
		for i < o.n_syms {
			...
			if sn > 0 && o.section_bases[sn] != nil {
				...
				// into the `.odinti` section under -livepatch. Their link names are
	...
	//    (exe PDB / export / __imp_ cell) via `lp_resolve`.
	...
	//      compiler-published `__odin_livepatch_type_table_ref`); `fresh_ti_hdr` is the
	...
	if meta_i >= 0 {
		...
		if lp_type_table_needs_swap(mo.data, mo.sym_off, mo.n_syms, mo.strtab_off, mo.section_bases) {
			...
			// publishes `__odin_livepatch_type_table_ref` (a pointer global holding that address)
			...
			if ref := lp_resolve_pdb("__odin_livepatch_type_table_ref"); ref != nil {
				...
				tt_ref = lp_resolve_pdb(LP_TYPE_TABLE_SYM) // ref global absent from PDB: resolve the global directly
			}
			if ok {
				fresh_ti_hdr = hdr
	...
	for h in hot_names {
		original := lp_resolve_pdb(h.name) // cached; found in PASS 1c
	...
	// `_lp_owner` with this serial, so a later reload knows when those references have all
	...
	// brand-new types. Tracked in `_lp_owner` so the block backing the fresh table is kept alive
	...
	// actually swapped — then `fresh_ti_hdr`'s block is pinned alive by `_lp_owner[tt_ref]`, so
	// the baseline's pointers stay valid. Done here, AFTER lp_resume: it allocates (heap
	...
	// no thread executes in them (see lp_try_free_old_generations). Pinned to the OS heap —
	// process-lifetime, caller-allocator-independent, like `_lp_syms`.
	{
		gen: Lp_Generation
...
PAD_LEN   :: 16 // pre-function pad reserved by `/FUNCTIONPADMIN:16` (see -livepatch linker flags)

LP_DEBUG_PAD :: #config(LP_DEBUG_PAD, false) // dump pad + entry bytes on each patch
...
// True iff the running procedure at `entry` is an `@(livepatch)` procedure. Such a
...
// Must be called with all other threads suspended (see `lp_suspend_others`) so the
...
// usable pad — e.g. the exe was not built with the `-livepatch` linker flags.
@(private)
...
// handles of the suspended threads; pass them to `lp_resume`.
@(private)
...
//   - direct Odin symbol      -> the exe's PDB (via `lp_resolve_external`)
...
// exe's PDB (see `lp_dbghelp_ensure`). Exact-match lookups here, NOT `SymFromNameW`:
...
lp_enum_cb :: proc "system" (pSym: win.PSYMBOL_INFOW, size: win.ULONG, user: win.PVOID) -> win.BOOL {
	...
	// via their `__odin_lptls$<name>` accessor, not by this direct entry).
	...
	if n > 0 {
		...
		// Keys live for the process lifetime alongside `_lp_syms`: pin to the OS heap.
...
// Initialize DbgHelp and enumerate the running exe's PDB symbols into `_lp_syms`.
...
lp_dbghelp_ensure :: proc() -> bool {
	...
	_lp_dbghelp_ready = true // attempt exactly once; a missing PDB will not appear later
	if !win.SymInitialize(win.GetCurrentProcess(), nil, true) {
		return false
	}
	win.SymSetOptions(win.SYMOPT_DEFERRED_LOADS)
	_lp_syms = make(map[string]rawptr, runtime.heap_allocator()) // process-lifetime: pin to the OS heap
...
// write before it would spill into the next procedure/symbol (see lp_patch_overwrite).
...
// `__odin_lptls$<varname>() -> rawptr { return &var }` whose body lowers to the exe's
...
lp_trampoline_for :: proc(na: ^Near_Arena, target: uintptr) -> rawptr {
	...
	thunk := lp_near_bump(na, 16) // 14 bytes rounded up
	if thunk == nil {
		return nil
	}
	lp_write_abs_jump(([^]u8)(thunk), rawptr(target)) // jmp qword ptr [rip+0]; .quad target
