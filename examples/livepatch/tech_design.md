# Odin livepatch — technical design

Live++-style in-process livepatch for Odin (Windows / x64). This document is the single
source of truth for why the feature works the way it does. The source (the compiler plus the
`core:livepatch` loader) keeps only short, local comments. Each such comment points back
here by section number.

The feature has two halves. They meet at a fixed set of emitted symbols.

- **Compiler** (`src/*.cpp`): under `-livepatch` it makes each user procedure patchable. It
...
- **Loader** (`core/livepatch/livepatch.odin`): at run time it maps a fresh object into
...
Section 13 lists the `__odin_livepatch_*` symbols that form the contract between the two halves.
...
Workflow:
    ...
    # build the host once
    odin build <pkg> -livepatch -debug
    # recompile after an edit (this does not restart the process)
    odin build <pkg> -livepatch-patch
    # from inside the running process
    livepatch.apply_dir("hot_objs")
...
| Flag | Effect |
| --- | --- |
| `-livepatch` | Build a host exe that supports in-process livepatch. |
| `-livepatch-patch` | Build the reload patch object for a running host. |
| `-livepatch-manifest:<path>` | Override the manifest path (section 3). |
| `-livepatch-arena-size:<int>` | Bytes reserved for new globals (default 262144). |
| `-livepatch-tls-arena-size:<int>` | Per-thread bytes reserved for new thread-locals (default 4096). |

`-livepatch` turns on these related settings:
...
`-livepatch-patch` is the whole "recompile" step. It turns on everything that `-livepatch`
...
section 14). So the recompile command is just `odin build <pkg> -livepatch-patch`. The user
...
`<main-package-dir>/odin-livepatch.manifest`. Both builds of the *same* package resolve to the
same default path, so the user need not pass `-livepatch-manifest`. Pass the same explicit path
...
build look like a reload. (`-livepatch` requires the host to be an exe, so a non-Object build is
always the base.)
...
the manifest. A running app can then rebuild the patch itself with `livepatch.build_patch()`,
which runs `odin build <pkg_dir> -livepatch-patch`.
...
- **New thread-locals** from a reload go into `__odin_livepatch_tls_arena`. This is a
...
- **Original thread-locals** stay preserved through a per-variable accessor thunk. The compiler
  emits one thunk for each:
      ...
      __odin_lptls$<link-name>() -> rawptr { return &var }
...
**Content hash** (`lb_livepatch_proc_content_hash`) is a stable, debug-normalized hash of a
...
**`__odin_livepatch_func_hashes`** is a table of `{u64 name_hash, u64 content_hash}` over every
livepatchable procedure. This is the same set that gets the patchable prologue. The compiler
...
when one of its procs changes, and proc hashing is enough. `lb_livepatch_skip_object` skips (1) a
...
**Build-id guard (F6).** `__odin_livepatch_build_id : u64` is a fingerprint of the exe's
...
**Signature changes (F8) — allowed, Live++ parity.** A hot proc *may* change its parameter and
return types across a reload. Redirection is at the callee's entry (the prologue pad's absolute
jump), and call sites are never rewritten, so the mechanism is identical to Live++'s hotpatch
jump. Safety follows the same way it does in Live++: the changed callee gets a new content hash
and is re-patched, and every *direct* caller's argument marshalling necessarily changes too, so
that caller's content hash changes, its module is emitted, and it is re-patched in the *same*
reload — reaching the new body through the new ABI. `apply_patch`/`apply_dir` apply the callee and
all such callers together as one `apply_many` reload, threads suspended, published atomically (the
sync point). The compiler already rejects any call expression that doesn't match the new
signature, so an old-ABI caller of a new-ABI body cannot be produced.

The base build still records each hot proc's canonical signature hash in `manifest.sig` (it feeds
`build_id`), but a signature change is **not** rejected. The one residual hazard is a stored
`proc`-value that still holds the old signature and is called through the old ABI — the identical,
documented caveat Live++ carries for raw function pointers. Unlike this, the global *layout* guard
of section 4 stays: a value global's storage is at a fixed address that unpatched code reads
through baked-in offsets, which the entry-jump never rewrites.

---

## 7. Patchable procedures

Under `-livepatch` the compiler makes **every eligible procedure hot-patchable, with no
`@(livepatch)` tag**. `lb_proc_is_livepatchable` defines eligibility. The compiler uses the same
...
- is not opted out with `@(no_livepatch)`;
...
user rarely edits those procedures during a livepatch session. The user edits their own code. The
...
- A user `-collection` (a collection named `core` included) has `builtin=false`. So a game
  engine's `src/engine/core` stays livepatchable.
- `ODIN_ROOT/examples/...` (the livepatch tests) is under no collection. So it stays
  livepatchable too.
...
`@(no_livepatch)` opts one procedure out. It stays inlinable, optimized, and never patched. The
...
Under `-livepatch` the optimization split is **fixed for any global `-o` flag**:
...
The split needs separate modules. So `-livepatch` forces `use_separate_modules` on
...
- **Full type-info expansion** (`generate_minimum_dependency_set`, gated on
  `livepatch && !no_rtti`): emit `type_info` for every declared entity's type, for the types
...
- **Published slice** (`lb_setup_type_info_data`): the loader needs both the exe's (old) and the
  ...
  `[]^Type_Info` slice under the fixed external name `__odin_livepatch_type_infos`. It shares the
...
link name in the PDB, so they resolve. Under `-livepatch` the compiler passes the link name as
both the debug name and the linkage name, so globals resolve too. A livepatch build is a dev
...
`__odin_livepatch_refresh_syms` as `{size, link name}`. The loader looks up each name in both the
...
object's static storage would be zero, which the refresh repoint cannot use. Under `-livepatch`
...
after. The expected shape is `proc(changed: []livepatch.Type_Change)`: one slice parameter, no
results. The checker enforces this loosely, because the loader casts the resolved address to this
type, and an odd signature would mis-call. Both attributes need a `-livepatch` build, and both
force `is_export` so the symbol has a stable, unmangled name.

The compiler emits two tables, `__odin_livepatch_pre_patch_hooks` and
`__odin_livepatch_post_patch_hooks`. Each lists the **exported symbol name** of every matching
...
Under `-livepatch` the linker gets:
...
under `-livepatch`, and the loader reads it by name. The loader finds some symbols in the exe's
...
| `__odin_livepatch_post_patch_hooks` | same as pre | Section 11. Post-patch hook names (resolved against the object). |
...
3. **`init_build_paths` filesystem side effects** (section 2). The `-livepatch-patch` out-dir
...
4. **Windows / x64 only.** The loader is `#+build windows`. The codegen paths assume the PE/COFF,
   ...
   work. Until then the `-livepatch` flag should be scoped to Windows/x64 at parse time.
