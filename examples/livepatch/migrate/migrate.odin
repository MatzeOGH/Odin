#+build windows
package livepatch_migrate

// Automated demo/test: react to a STRUCT LAYOUT CHANGE across a livepatch, using
...
// only the livepatched procs do — so the host binary is layout-agnostic. On reload:
...
//     saved fields into it, reflecting via livepatch.Type_Change.new (the reloaded
...
State :: struct {
	// LP_EXTRA_FIELD  (the reload edit replaces this line with `extra: i64,`)
	n:    i64,
	tag:  Kind,
	vals: [3]i64,
}
