...
#
# Builds the .exe (-livepatch, State v1), simulates an edit that inserts a new field
...
Write-Host '==> building migrate-test .exe (-livepatch, State v1)'
& $odin build $here -out:(Join-Path $work 'migrate_test.exe') -debug -livepatch -livepatch-manifest:$manifest -ignore-unknown-attributes
...
& $odin build $src -build-mode:obj -use-single-module -livepatch -livepatch-manifest:$manifest -out:(Join-Path $work 'migrate_hot.obj') -ignore-unknown-attributes
