# Automated test: MULTI-PACKAGE livepatch — one reload patches every user package that
# changed, across a cross-package call chain, while an unchanged package is not re-emitted.
#
#   powershell -ExecutionPolicy Bypass -File examples\livepatch\multipkg_test\run_multipkg_test.ps1
#
...
Write-Host '==> building multipkg .exe (-livepatch, base versions)'
& $odin build $here -out:(Join-Path $work 'multipkg.exe') -livepatch -livepatch-manifest:$manifest
...
foreach ($mustnot in 'hot-pkgd.obj','hot-livepatch_multipkg.obj') {
