# Live++-style in-process livepatch (proof of concept)
...
`-livepatch` every procedure is hot-patchable automatically (Live++-style). On
...
1. **Build the demo once and run it.** `-livepatch` reserves the new-global arena,
   ...
   ```
   odin build examples/livepatch -out:examples/livepatch/livepatch.exe -livepatch
...
2. **Edit** `examples/livepatch/game.odin` — change the body of `update`, and
...
`livepatch.apply_patch()`, which runs `odin build <pkg> -livepatch-patch` for you
...
the `-livepatch` exe build recorded in the manifest; `odin` is taken from `PATH` unless
...
```
powershell -ExecutionPolicy Bypass -File examples\livepatch\demo.ps1
```
...
was written correctly too. The `[livepatch] fmt from hot code …` line is printed **by the
patched `update` itself** — `fmt` of ints now works from inside hot code. (The
`[livepatch] note: 0 unresolved and N unsupported …` line is expected: `unsupported`
...
1. **The build flag (no tags).** Building with `-livepatch` makes **every**
   eligible procedure replaceable — no `@(livepatch)` tag. The compiler gives each
   one `noinline` and a patchable prologue (a 16-byte pad + short-redirect entry);
   `@(no_livepatch)` opts a procedure out. `-livepatch` implies `-debug`.
...
4. **Recompile to an object.** On reload the program is rebuilt to reload object(s)
   ...
   (`-livepatch -build-mode:obj -livepatch-manifest:<path> -out:<dir>`) still work
   and are needed only when the exe and the reload build come from different directories.

5. **Load + relocate + patch** (`core:livepatch`, call `livepatch.apply("hot.obj")`):
...
`examples/livepatch/mt_test` program is an automated regression test for this
...
- `game.odin` — the demo: an ordinary (untagged) procedure `update` and a `main`
  REPL where `b` = `livepatch.apply_patch` (rebuild + reload in-process) and `r` =
  `livepatch.apply_dir` (reload only).
...
- The loader itself now ships as **`core:livepatch`** (no longer copied here).
...
- Mark a proc `@(pre_patch_hook)` or `@(post_patch_hook)`. The loader **autowires**
  them — no registration call. Signature: `proc(changed: []livepatch.Type_Change)`.
...
```
powershell -ExecutionPolicy Bypass -File examples\livepatch\migrate\run_migrate_test.ps1
powershell -ExecutionPolicy Bypass -File examples\livepatch\migrate_enum\run_migrate_enum_test.ps1
...
- `-livepatch` (implies `-debug`), `-livepatch-arena-size`, `-livepatch-manifest`
  ...
  (`SymEnumSymbolsW`).
- Reflection from hot code — under `-livepatch`, a complete `type_table`
...
- `@(rodata)` / `#load` / `#load_directory` / `#hash` / `#load_hash` refresh — immutable
  ...
  the reload object's fresh copy (`__odin_livepatch_refresh_syms` table + the loader's
  repoint pass; `#load_directory` globals are baked const under `-livepatch` via
  ...
  `core/livepatch/livepatch.odin`.
- `core:livepatch` — the COFF loader/relocator/patcher + `apply`.
- `@(pre_patch_hook)` / `@(post_patch_hook)` attributes + emitted hook-name tables
  (`__odin_livepatch_{pre,post}_patch_hooks`, storing each hook's exported symbol name):
  `src/checker.{hpp,cpp}`, `src/check_decl.cpp`, `src/entity.cpp`, `src/llvm_backend.cpp`;
  the new-layout type table (`__odin_livepatch_type_infos`) in `src/llvm_backend_type.cpp`;
