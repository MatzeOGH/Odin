...
//   (4) the ordinary livepatch patch still takes effect.
...
State :: struct {
	n:    i64,
	mode: Mode,
	// LP_EXTRA_FIELD  (the reload edit inserts `extra: i64,` here -> layout change)
}
...
main :: proc() {
	...
	// The livepatch patch must also have taken: reloaded body adds 100.
	probe: State
	PROBE :: 10
	for _ in 0 ..< PROBE {
		work(&probe)
