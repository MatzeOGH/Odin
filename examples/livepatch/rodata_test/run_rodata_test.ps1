# Automated regression test for @(rodata) and #load livepatch (Windows / x64).
#
#   powershell -ExecutionPolicy Bypass -File examples\livepatch\rodata_test\run_rodata_test.ps1
#
# Builds the test .exe (-livepatch), simulates an "edit" that changes a @(rodata) value
...
Write-Host '==> building rodata-test .exe (-livepatch -debug)'
& $odin build $here -out:(Join-Path $work 'rodata_test.exe') -debug -livepatch -livepatch-manifest:$manifest
...
& $odin build $src -build-mode:obj -use-single-module -livepatch -livepatch-manifest:$manifest -out:(Join-Path $work 'rodata_hot.obj')
