...
#
#   powershell -ExecutionPolicy Bypass -File examples\livepatch\mt_test\run_mt_test.ps1
#
# Builds the test .exe (-livepatch), simulates an "edit" that changes the
# livepatched `work` body from `s.n += 1` to `s.n += 100`, compiles that to
...
Write-Host '==> building mt-test .exe (-livepatch: bakes the table, hotpatch pad, /OPT:NOICF)'
& $odin build $here -out:(Join-Path $work 'mt_test.exe') -debug -livepatch -livepatch-manifest:$manifest
...
& $odin build $src -build-mode:obj -livepatch -livepatch-manifest:$manifest -out:(Join-Path $objs 'mt_hot.obj')
