...
$manifest = Join-Path $work 'migrate_enum.manifest'
...
& $odin build $exeSrc -out:(Join-Path $work 'migrate_enum.exe') -debug -livepatch -livepatch-manifest:$manifest -ignore-unknown-attributes
if ($LASTEXITCODE -ne 0) { throw 'exe build failed' }

Write-Host '==> compiling v2 layout to migrate_enum_hot.obj (same manifest)'
$objSrc = Stage 'obj_src' 'v2'
& $odin build $objSrc -build-mode:obj -use-single-module -livepatch -livepatch-manifest:$manifest -out:(Join-Path $work 'migrate_enum_hot.obj') -ignore-unknown-attributes
if ($LASTEXITCODE -ne 0) { throw 'obj build failed' }

Write-Host '==> running'
Push-Location $work
try {
	& (Join-Path $work 'migrate_enum.exe')
	$code = $LASTEXITCODE
}
