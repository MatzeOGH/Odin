...
# livepatch feature end to end (Windows / x64).
#
#   powershell -ExecutionPolicy Bypass -File examples\livepatch\demo_raylib.ps1
#
...
Write-Host '==> building demo .exe from current source (links raylib.lib; -livepatch auto-adds /OPT:NOREF,NOICF to keep unreferenced raylib functions)'
# -livepatch implies -debug and auto-adds /OPT:NOREF,NOICF, so neither is passed here.
# (Add -extra-linker-flags:"/WHOLEARCHIVE:raylib.lib" only to reach a member the base never pulls.)
& $odin build $here -out:(Join-Path $work 'livepatch.exe') -livepatch -livepatch-manifest:$manifest
if ($LASTEXITCODE -ne 0) { throw 'exe build failed' }
if (-not (Test-Path (Join-Path $work 'livepatch.pdb'))) { throw 'no PDB produced next to the exe (need -debug)' }
...
update :: proc(s: ^State) {
	...
	s.mirror = i64(r)
	fmt.println("   [livepatch] raylib from hot code -- GetRandomValue(1,100) =", r)
}
...
& $odin build $v2 -build-mode:obj -use-single-module -livepatch -livepatch-manifest:$manifest -out:(Join-Path $work 'hot.obj')
if ($LASTEXITCODE -ne 0) { throw 'obj build failed' }

Write-Host '==> running (tick, reload, tick, quit)'
Write-Host '    after the reload, expect:  reload ok: true  and a "[livepatch] raylib from hot code -- GetRandomValue(1,100) = N" line'
...
Push-Location $work
try {
	"t`nr`nt`nq" | & (Join-Path $work 'livepatch.exe')
}
