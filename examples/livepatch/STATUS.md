...
- [x] **Hot-procedure signature/ABI guard.** Existing *globals* are protected against a
      ...
      manifest (`sig` records); a reload build recomputes it in `lb_livepatch_emit_func_hashes`
      ...
      `livepatch_manifest_write`), so the guard is not "sticky" after a revert. Verified:
      ...
      `src/llvm_backend.hpp` (`LivePatchManifest.sig`). (Adversarial-review finding F8.)
