# Livepatch — status & TODO

Live++-style in-process livepatch for the Odin compiler. Windows / x64.
...
- [x] **Robust across MANY reloads: the type baseline now advances every reload (and the reflection
      ...
      - **Frozen type baseline.** The loader diffed each reload's fresh type-info against a map built
        ONCE from the exe's baked table (`lp_exe_types_cache`, never updated) — unlike the per-proc
        `_lp_cur` hash baseline, which already advanced each reload. So on reload #2+ every
        ...
        *added* on reload N was treated brand-new forever, so a later change to it was silently
        skipped. **Fix:** `lp_exe_types_cache` → `_lp_live_types`, seeded from the exe once then
        rebuilt from each reload's fresh, complete table (`lp_advance_live_types`, called after the
      ...
      - **Reflection swap silently never fired.** `lp_resolve_pdb("__odin_livepatch_type_table_ref")`
        returned nil on this toolchain — that constant *pointer* global is not surfaced by the exe's
        PDB symbol enumeration (the sibling `__odin_livepatch_type_infos` *slice* global is), so the
        ...
        directly by its enumerated link name (`LP_TYPE_TABLE_SYM`) when the ref global is absent.
      Verified: new `examples/livepatch/migrate_twice` (exe v1, reloads v2 then v3) asserts reload
...
- [x] **Reloading broke static string literals (garbage / wrong strings after a reload)** and, once
      ...
      - **Fix (future-proof, section-based).** The compiler now tags every fresh-per-reload private
        constant with the `.odinti` section under `-livepatch` — done once in
        ...
        (`core/livepatch/livepatch.odin`, `lp_is_object_local_const`) classifies **by section**:
...
- [x] **Reflection now refreshes across a reload (edited layouts AND brand-new types).** Previously
      ...
      `len` dynamically, the loader can simply swap the table. Under `-livepatch` the compiler
      publishes the address of `runtime.type_table` as `__odin_livepatch_type_table_ref`
      ...
      under the stop-the-world + `VirtualProtect`, tracked in `_lp_owner` so the backing block is kept
      alive. A type-only reload (no proc/data change) no longer early-frees, so it still performs the
      swap. Loader: `lp_type_table_needs_swap` + the swap in `apply_many`. Verified end-to-end by
      `examples/livepatch/reflect_test` (adds `y: i64` to a struct and a brand-new `Extra` type;
...
- [ ] Odin has a concept of LibraryCollections, it should be possible to disable an enable livepatch selectifly for them


## Done

- [x] **Separate modules + per-module optimization + multi-object incremental reload.**
      `-livepatch` no longer forces `-use-single-module` (`src/main.cpp`); at `-o:none` the
      ...
      - **Minimal object emission** (`lb_livepatch_skip_object` in `src/llvm_backend.cpp`): a
        ...
        the manifest baseline — `lb_livepatch_emit_func_hashes` sets `lbModule.livepatch_changed`)
        is skipped too.
      - **Multi-object loader** (`core/livepatch/livepatch.odin`): `apply` →
        ...
        (`lbLivePatchStaticSym.module`).
      - **Auto-manifest:** `-livepatch-manifest` is now optional — it defaults to
        `<main-package-dir>/odin-livepatch.manifest` (`src/build_settings.cpp`). Base-vs-reload
        ...
        never makes it masquerade as a reload (`livepatch_manifest_read` early-returns unless
        `build_mode == BuildMode_Object`). No manual manifest path or reset is needed for a
        same-directory build; pass `-livepatch-manifest` explicitly only when the exe and the
...
- [x] **`@(livepatch)` procedure attribute** — marks a procedure replaceable:
      ...
      - `src/checker.hpp` (AttributeContext), `src/checker.cpp` (proc_decl_attribute),
        `src/entity.cpp` (Entity.Procedure.is_livepatch), `src/check_decl.cpp`,
        `src/llvm_backend_proc.cpp`.
- [x] **`-livepatch` build flag** — forces single module; reserves the arenas + emits the
      ...
      - `src/main.cpp` (BuildFlag_LivePatch + the -debug check), `src/build_settings.cpp` (bool livepatch).
- [x] **PDB-based symbol resolution** (replaced the baked `runtime.livepatch_symbol_table`).
      ...
      - `core/livepatch/livepatch.odin` (`lp_dbghelp_ensure` enumerates, `lp_resolve_pdb`),
        `core/sys/windows/dbghelp.odin` (`SymEnumSymbolsW`),
        `src/llvm_backend.cpp` (`lb_livepatch_emit_support`).
- [x] **`FlushInstructionCache` binding** — `core/sys/windows/kernel32.odin`.
- [x] **COFF object loader / relocator / patcher** — `core/livepatch/livepatch.odin`:
...
- [x] **Loader promoted to `core:livepatch`** with `apply(obj_path)` that
      auto-discovers the `@(livepatch)` procedures from the baked table (no more
...
- [~] **Incremental compilation (object side done; front-end still whole-program).**
      `-livepatch` no longer forces `-use-single-module`: both the base exe and the reload
      ...
      (`lb_livepatch_skip_object`) every builtin-collection module (base/core/vendor: already
      ...
      `lb_livepatch_emit_func_hashes`; a module is flagged `lbModule.livepatch_changed`). So a
...
- [x] **Thread-safe / atomic patching.** Patching is now safe while other threads run the
      ...
      stress test: `examples/livepatch/mt_test` spawns worker threads that hammer the hot
  ...
  - [x] **Stop-the-world + IP check.** The loader enumerates every other thread
        ...
        only new calls take the jump. Loader: `lp_suspend_others`/`lp_resume`/
        `lp_ip_conflicts` in `core/livepatch/livepatch.odin`; APIs in
        `core/sys/windows` (added the native x64 `CONTEXT_*` flag constants).
  - [x] **Atomic short-jump publish.** Each `@(livepatch)` proc gets a 16-byte NOP pad
        ...
        world). Loader: `lp_patch_atomic`/`lp_patch_overwrite`/`lp_write_abs_jump`.
- [x] **Unwind info for new code.** The loader now fixes up `IMAGE_REL_AMD64_ADDR32NB`
      ...
      `core/livepatch/livepatch.odin` (ADDR32NB case in the relocation switch +
...
- [x] **New calls into an already-linked package/foreign library.** Reloaded code may now
      ...
      `lp_resolve_external` that looks the symbol up in the running exe's PDB via DbgHelp
      `SymFromNameW` (`core/livepatch/livepatch.odin` `lp_resolve_pdb` +
      `lp_dbghelp_ensure`; new `SymFromNameW` binding in `core/sys/windows/dbghelp.odin`).
      ...
      - **Verified:** `examples/livepatch/demo_raylib.ps1` — base references
...
- [x] **handle `@(rodata)`, `#load`, `#load_directory`, `#hash`, `#load_hash`.** Immutable embedded data — `@(rodata)` globals and
      ...
      its `{link name, size}` in a self-contained `__odin_livepatch_refresh_syms` table; the loader
      **overwrites the exe's canonical copy** with the reload object's fresh copy under the same
      ...
      Compiler: `src/llvm_backend.cpp` (`lb_is_livepatch_refresh_global` /
      `lb_is_load_directive_expr`, the fork exemption, the `__odin_livepatch_refresh_syms` table),
      `src/llvm_backend_stmt.cpp` (same for local `@(static)`), `src/llvm_backend.hpp`
      (`lbLivePatchRefreshSym`). Loader: `core/livepatch/livepatch.odin` (refresh-target
      ...
      repoint); under `-livepatch` the compiler instead bakes its constant `[]Load_Directory_File`
      ...
      Verified end-to-end by `examples/livepatch/rodata_test` (`run_rodata_test.ps1`, exit 0 = PASS):
...
- [x] **General external-symbol resolution.** Undefined externals the reload object
      ...
      near-block absolute-jump trampoline. Loader: `core/livepatch/livepatch.odin`
      (`lp_resolve`, `lp_resolve_exported`, `lp_imp_cell`, `lp_trampoline_for`, `Near_Arena`).
...
- [x] **Hot-procedure signature changes — allowed (Live++ parity).** A hot proc's
      parameter/return types *may* change across a reload; the build-time rejection that once
      lived here (the F8 ABI guard) has been **removed**. Redirection is at the callee's entry and
      call sites are never rewritten, so a signature change is an ordinary code change: the changed
      callee re-patches, and every *direct* caller's argument marshalling changes with it, so the
      caller re-patches in the same `apply_many` reload and reaches the new body through the new
      ABI. The compiler already rejects any call expression that doesn't match the new signature,
      so an old-ABI caller of a new-ABI body cannot be produced. The base build still records each
      hot proc's canonical signature hash (`type_hash_canonical_type(p->type)`) in the manifest
      (`sig` records) because `build_id` folds it in, but `lb_livepatch_emit_func_hashes` no longer
      compares or errors on it. Residual caveat (same as Live++'s raw-function-pointer note): a
      stored `proc`-value holding the old signature, called through the old ABI. The global
      *layout* guard is unrelated and stays. Verified: changing `add :: proc(x: int)` to
      `proc(x, y: int)` (with its caller updated) reloads and returns the new-ABI result
      (`tests/livepatch`, `sig_v1`→`sig_v2`). Compiler: `src/llvm_backend.cpp`. (Was
      adversarial-review finding F8; intentionally lifted for Live++ parity.)
- [x] **Variadic / `any` / reflection from hot code.** Both the crash (unresolved
      ...
      `-livepatch`-gated compiler changes make it work: (1) `src/checker.cpp`
...
- [x] **Thread-local variables from hot code.** Reads/writes of a *pre-existing*
      ...
      (`__odin_lptls_<i>() -> rawptr { return &var }`, whose body lowers to the exe's TLS
      sequence) and a `LIVEPATCH_KIND_TLS` symbol-table entry; the loader reads the
...
- [x] **New thread-locals across a reload.** A `@(thread_local)` variable introduced
      ...
      reserves a per-thread **TLS arena** (`__odin_livepatch_tls_arena: [N]u8`,
      `-livepatch-tls-arena-size`, default 4 KiB) so every thread — existing and
      ...
      clear build error naming `-livepatch-tls-arena-size`.
- [ ] **livepatch hooks** — After a livepatch the structure of objecst can be different.
			...
			of where objects live is not visible to the livepatch function the user has to manually
...
- [x] **Drop the baked table / binary bloat** — the exe no longer bakes
      `runtime.livepatch_symbol_table` (which took the address of every proc/global and so
      ...
      recovers the `@(livepatch)` set **structurally** from the 16-byte patchable-function
      prefix (no table, no metadata section). LLVM-level DCE is re-enabled. `-livepatch` now
      **requires `-debug`** (a PDB must sit next to the exe; enforced in `src/main.cpp`).
      ...
      external linkage; a global's debug name is set to its link name under `-livepatch` so
      the PDB carries a resolvable name (functions already do). Files: `base/runtime/core.odin`
      (types + slice removed), `src/llvm_backend.cpp` (`lb_livepatch_emit_support` replaces
      `lb_generate_livepatch_symbol_table`; global debug-name change), `src/main.cpp`,
...
- [x] **`/OPT:NOREF,NOICF` when livepatching** — `/OPT:NOICF` stops the linker folding
      ...
      them via the PDB. Both are now set **automatically** under `-livepatch` (`src/linker.cpp`,
...
- [x] **No `@(livepatch)` attribute (tagless, Live++-style)** — the user no longer tags
      procedures. Under `-livepatch` every eligible procedure **in the user's own packages** is
      made hot-patchable automatically (`lb_proc_is_livepatchable` in `src/llvm_backend_proc.cpp`
      gates the `noinline` + patchable-prologue emission; eligible = has a body, not foreign, not
      the entry point, not force-inlined, normal prologue, not `@(no_livepatch)`, **and not in the
      ...
      **collection exclusion is essential for compile memory**: making EVERY proc in the program
      ...
      examples/...` (the livepatch tests) is under no collection, so it stays reloadable too.
      Verified: `mt_test` 200/200, and a proc in `.../src/engine/core` livepatchs. A second memory
      fix: the per-proc content-hash (`lb_livepatch_proc_content_hash`) now frees its normalized-IR
      ...
      `@(livepatch)` attribute is REMOVED; `@(no_livepatch)` opts a specific proc out (keeps
      ...
      `{u64 name_hash, u64 content_hash}` table `__odin_livepatch_func_hashes` (no addresses,
      DCE stays on) via `lb_livepatch_emit_func_hashes`, where `content_hash` is a debug- and
      ...
      (`core/livepatch/livepatch.odin`) seeds a live `name_hash→content_hash` map from
      ...
      "patch the patcher" hazard and only edited procs relocate). `-livepatch` now **implies
...
- [x] **Optimized builds** (`-o:speed`/`-o:size`) — supported. Under `-livepatch` the per-module
      ...
      `-livepatch`), and `src/build_settings.cpp` (`init_build_context` — force
      `use_separate_modules` on under `-livepatch` so the split holds at `-o:speed`/`-o:size`,
      ...
      - **Why this over matching opt levels in the manifest:** an earlier approach let user code
        ...
        `-livepatch-patch` recompile (always `-o:none`) is correct against a base built at ANY
        `-o`. Relocation/patch behavior at `-o:speed` is now identical to `-o:none` (user IR is the
        same); only the stdlib is optimized.
      - **Verified** (`mt_test`, Windows/x64): base `-livepatch -o:speed` (and `-o:size`) with a
...
- [x] **Free the previously mapped block** — a reload no longer leaks its mapped block(s) +
      `.pdata` registration. Two paths, both in `core/livepatch/livepatch.odin`:
      ...
      - **Patching reload:** each reload's allocations (object block, trampoline/import-cell
        ...
        proc entry or refreshed `@(rodata)`/`#load` header that still names it (`_lp_owner` maps
      ...
      - Verified by `examples/livepatch/free_test` (`run_free_test.ps1`, exit 0 = PASS): 200
...
- [x] **Guard the 14-byte patch** — the non-atomic prologue overwrite (`lp_patch_overwrite`)
      ...
      (`!lp_has_patch_pad`) **and** whose gap to the next enumerated PDB symbol
      (`lp_next_symbol_after`, an O(n) min over `_lp_syms`) is `< PATCH_LEN` — aborting the whole
      ...
      `lp_patch_overwrite` keeps the same check defensively for any direct caller. Verified:
      `mt_test` still 200/200 (every proc has its pad, so the guard never trips on the normal
      path). Loader: `core/livepatch/livepatch.odin` only.
- [x] **Build-identity / staleness check (safety net).** A reload object is now bound to the
      *running* exe. The compiler bakes `__odin_livepatch_build_id : u64` (external,
      ...
      (`core/livepatch/livepatch.odin`, before any write) compares the exe's id (PDB) with
      ...
      Compiler: `src/llvm_backend.cpp` (`lb_livepatch_emit_build_id`), `src/llvm_backend.hpp`
      (`LivePatchManifest.build_id`). (Adversarial-review finding F6.)
- [x] **Hardened PDB-resolver failure modes (from adversarial review).** (a) The loader now
      ...
      instead of silently duplicating every global (`lp_dbghelp_ensure`). (b) TLS/register/
      frame/value symbols (whose `Address` is not a VA) are skipped during enumeration so
      they cannot poison the map (`lp_enum_cb`). (c) A reload aborts before writing any byte
      ...
      rather than writing a wrapped RVA. (f) `-livepatch` host is required to be an
      ...
      process-lifetime state (`_lp_syms`/`_lp_cur`/`_lp_live_types` + their string keys) is
      ...
      app's `context.temp_allocator`; and a `_lp_busy` atomic guard makes a concurrent/nested
      `apply()` fail loudly instead of corrupting shared state (F14).


### Tier 4 — workflow / reach

- [x] **`-livepatch-patch` recompile flag (CLI ergonomics).** The reload/recompile step is now
      a single, self-documenting command: `odin build <pkg> -livepatch-patch`. The flag implies
      `-livepatch` (which itself implies `-debug` + auto `/OPT:NOREF,NOICF`) **plus**
      ...
      build error. The host build likewise drops to `odin build <pkg> -livepatch` (the old
      ...
      `lp.apply_dir()`. All five livepatch flags now have `odin build --help` text (previously
      ...
      `src/main.cpp` (`BuildFlag_LivePatchPatch` enum/registration/handler, the help entries, the
      contradiction guard), `src/build_settings.cpp` (`livepatch_patch` field, the out-dir
...
- [ ] **Built-in file watching + auto-rebuild** — watch sources, rebuild the object,
      and reload automatically (today: `lp.apply_patch()` or press `b`/`r`, no file watcher yet).
- [x] **Self-contained reload** — the running program drives the rebuild itself, so no external
      terminal is needed. `core:livepatch` gained `build_patch()` and `apply_patch()`:
      `build_patch` shells out (via `core:os` `process_exec`) to `odin build <pkg_dir>
      -livepatch-patch`, streaming the compiler's stdout/stderr; `apply_patch` runs that and, only
      ...
      `<pkg_dir>` is the absolute base-package directory the `-livepatch` exe build now records in
      the manifest (`pkg_dir` line, written by `livepatch_manifest_write`, preserved across reload
      builds via `LivePatchManifest.pkg_dir`), so the app needs no build config of its own — it
      ...
      manifest field/read/write). Loader: `core/livepatch/livepatch.odin`
      (`build_patch`/`apply_patch`/`lp_run_patch_build`/`lp_manifest_pkg_dir`). Verified end-to-end:
...
- [ ] **Setup test framework** — one framework for testing all livepatch features and
			edge cases. It has to be repeatable and verify each aspect of the livepatch.
- [ ] **thread_local** — all flavors of possible thread locals have to be tesed and
			verified that they work correctly over livepatchs
...
- **`-livepatch` implies `-debug`.** The loader resolves the running exe's symbols from its
  PDB, so a `.pdb` must sit next to the exe; `-livepatch` now turns on `-debug` automatically
  (`src/main.cpp`). The hot-patchable set is emitted for every eligible procedure and recovered
  structurally from the patchable-function prefix — see `lp_is_hot_entry`. The global `-o` flag
  is honored for the **standard library only**: under `-livepatch`, user-editable code
  ...
  reload compile user code identically and `-o:speed`/`-o:size` still livepatch (separate
...
- **Stale procedure pointers.** Only direct call sites and the patched hot-proc entries
  ...
  *non-hot* proc which was recompiled is now stale; a stored pointer to a *hot* proc's entry
  keeps working (the entry is patched to jump). Keep dispatch tables pointing at `@(livepatch)`
...
- **Globals show under their mangled link name in the debugger (`-livepatch` builds only).**
  ...
  (`pkg::hits`) instead of its source identifier (`hits`) under `-livepatch`
  ...
  build WITHOUT `-livepatch` are unchanged; there is no effect on codegen/layout/ABI/runtime.
  Possible refinement: give globals external (public) linkage under `-livepatch` instead — the
...
(diff in `core/livepatch/livepatch.odin`: `lp_layout_differs` / `lp_contains_changed` /
`lp_build_type_changes`; serializer in `examples/livepatch/migrate/serializer.odin`, copied
...
- **Same-size field-type substitution** (`x: i32 → x: f32`, or `x: Foo → x: Bar` at the same
  size/offset/name). `lp_layout_differs` compares struct fields by name + offset only (not
...
`runtime.livepatch_symbol_table`). `[x]` = fixed, `[ ]` = still open. Line refs are
approximate against `core/livepatch/livepatch.odin` unless noted.

- [x] **F1 — HIGH — `SymEnumSymbolsW` result ignored → partial/empty map → silent state
      loss.** A missing/public-only PDB or a part-way enumeration left `_lp_syms` empty while
      `lp_dbghelp_ensure` still reported success, so every pre-existing symbol resolved to a
      fresh object-local copy (state reset) with only a misleading "no @(livepatch)
      procedures found" message. FIX: `lp_dbghelp_ensure` now checks the `SymEnumSymbolsW`
...
- [ ] **F2 — HIGH (fragility) — hot detection assumes the 16-byte prefix is literal `0x90`
      NOPs, and is stricter than the patcher's own pad check.** `lp_is_hot_entry` requires all
      16 pre-entry bytes == `0x90` (or a leading `FF 25`), while `lp_patch_atomic`'s
      `lp_has_patch_pad` also accepts `0xCC` over 14 bytes. A future LLVM emitting multi-byte
...
- [x] **F3 — MEDIUM — TLS/register/frame/value symbols poison `_lp_syms` with a non-VA
      `Address`.** `lp_enum_cb` stored `pSym.Address` unconditionally; for a thread-local that
      is a TLS-relative offset, not a pointer. Dodged today (TLS goes via accessors) but a
      live landmine. FIX: `lp_enum_cb` skips symbols flagged `SYMFLAG_TLSREL/REGISTER/REGREL/
...
- [x] **F5 — MEDIUM — loader hardcodes the exe module but the `-debug` gate admitted
      DLL/lib hosts.** `alloc_near_exe`/`lp_resolve_exported`/enumeration all use
      ...
      livepatch host to be `-build-mode:exe` (reload objects use `:obj`).
- [x] **F6 — MEDIUM — no build-identity check between reload object, manifest, and running
      exe.** FIXED: the compiler bakes `__odin_livepatch_build_id` (a fold of the exe's
...
- [~] **F8 — MEDIUM — no signature/ABI guard on hot procedures.** Originally "fixed" by
      rejecting a proc signature change at build time. **Superseded / intentionally lifted for
      Live++ parity:** signature changes are now allowed and reload normally, because the changed
      callee and every direct caller re-patch together in the same reload (the caller's marshalling
      changes, so its content hash changes). The rejection has been removed from
      `lb_livepatch_emit_func_hashes`; the `sig` hash is still recorded (it feeds `build_id`) but no
      longer compared. Residual caveat: a stored `proc`-value with the old signature. See the Tier 2
      "Hot-procedure signature changes" item.
...
- [x] **F13 — LOW/MEDIUM — process-lifetime state aliased the caller's allocator.** `_lp_syms`,
      `_lp_cur`, and `_lp_live_types` (backing store **and** string keys) were allocated from
...
- [x] **F14 — LOW/MEDIUM — `apply()` not re-entrant / thread-safe.** FIX: a `_lp_busy` atomic
