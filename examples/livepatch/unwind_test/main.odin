#+build windows
package unwind_test
// Verifies x64 stack unwinding works THROUGH livepatched code. On x64 there is no
...
// the dynamic function tables the livepatch loader registers (RtlAddFunctionTable). If the
...
main :: proc() {
	...
	if !lp.apply_dir("objs") { fmt.eprintln("UNWIND-TEST: FAIL (reload did not apply)"); os.exit(1) }
