...
@(pre_patch_hook)
on_pre :: proc(changed: []lp.Type_Change) {
	pre_fired += 1
	if g_state != nil {
...
@(post_patch_hook)
on_post :: proc(changed: []lp.Type_Change) {
	post_fired += 1
...
main :: proc() {
	...
	g_pass = 1
	if !lp.apply("mt1_hot.obj") { fail("reload 1 did not apply (is mt1_hot.obj present, same manifest?)") }

	fmt.println("== reload 2 (v2 -> v3) ==")
	g_pass = 2
	if !lp.apply("mt2_hot.obj") { fail("reload 2 did not apply (is mt2_hot.obj present, same manifest?)") }
	...
	if lp.live_generations() > 3 {
		fail(fmt.tprintf("reload generations not reclaimed: %d live (expected <= 3)", lp.live_generations()))
	}

	fmt.printfln("MIGRATE-TWICE: PASS (v1=%d v2=%d v3.old=%d onlyv2_reflagged=%v newv2_flagged=%v n=%d gens=%d)",
		v1_state_size, v2_state_new_size, v3_state_old_size, v3_onlyv2_flagged, v3_newv2_flagged, mig_n, lp.live_generations())
	os.exit(0)
}
