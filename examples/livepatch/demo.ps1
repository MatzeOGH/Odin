# Reproduces the livepatch proof of concept end to end (Windows / x64).
#
...
# single `-livepatch-patch` command, and runs the exe which reloads the patch into itself while
...
main :: proc() {
	...
	fmt.printfln("livepatch demo — pid %d", pid)
	...
	for {
		...
		for line in strings.split_lines_iterator(&chunk) {
			...
			case "q":
				return
			case "r":
				ok := lp.apply_dir() // default dir "hot_objs"
...
Write-Host '==> building demo .exe from the v1 base (-livepatch reserves the arena; implies -debug + auto /OPT:NOREF,NOICF)'
Push-Location $src
try {
...
update :: proc(s: ^State) {
	...
	fmt.println("   [livepatch] fmt from hot code -- reloads =", reloads, "threshold =", threshold)
	...
	fmt.printfln("   [livepatch] composite reflection -- state = %v, typeid = %v", s^, typeid_of(State))
}
...
Write-Host '==> rebuilding the reload PATCH with one command: odin build . -livepatch-patch'
# Same directory as the exe build, so no -build-mode/-out/manifest to align: -livepatch-patch
...
Get-Content (Join-Path $src 'odin-livepatch.manifest') | Select-String 'reloads|threshold|limits|next_free'
...
Write-Host '    after each reload the patched update prints a "[livepatch] fmt from hot code" line => fmt/any works from hot code.'
Push-Location $src
try {
	"t`nt`nr`nt`nr`nt`nq" | & (Join-Path $src 'livepatch.exe')
}
finally { Pop-Location }
