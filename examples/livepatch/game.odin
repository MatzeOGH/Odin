...
// With the exe built `-livepatch`, EVERY procedure is livepatchable automatically (no
...
update :: proc(s: ^State) {
	// ---- EDIT HERE, then `odin build . -livepatch-patch` (ctrl-alt-r) and press `r` ----
...
// The reload set's per-package objects live in `hot_objs\` (the default `-livepatch-patch`
// output dir, see recompile.ps1); `lp.apply_dir()` reads that directory by default.
...
main :: proc() {
	...
	fmt.println("`-livepatch-patch` build for you and reloads) — or build by hand and press `r`.")
	...
	for {
		...
		for line in strings.split_lines_iterator(&chunk) {
			...
			case "b":
				...
				ok := lp.apply_patch(odin = "..\\..\\odin.exe")
