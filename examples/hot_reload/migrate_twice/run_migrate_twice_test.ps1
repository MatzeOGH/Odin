...
#
#   powershell -ExecutionPolicy Bypass -File examples\hot_reload\migrate_twice\run_migrate_twice_test.ps1
#
...
& $odin build $exeSrc -out:(Join-Path $work 'migrate_twice.exe') -debug -hot-reload -hot-reload-manifest:$manifest -ignore-unknown-attributes
if ($LASTEXITCODE -ne 0) { throw 'exe build failed' }

Write-Host '==> compiling reload 1 (v2 layout) to mt1_hot.obj (same manifest)'
$obj1Src = Stage 'obj1_src' 'v2'
& $odin build $obj1Src -build-mode:obj -use-single-module -hot-reload -hot-reload-manifest:$manifest -out:(Join-Path $work 'mt1_hot.obj') -ignore-unknown-attributes
...
& $odin build $obj2Src -build-mode:obj -use-single-module -hot-reload -hot-reload-manifest:$manifest -out:(Join-Path $work 'mt2_hot.obj') -ignore-unknown-attributes
