...
#
#   powershell -ExecutionPolicy Bypass -File examples\hot_reload\free_test\run_free_test.ps1
#
# Builds the host (-hot-reload), then builds TWO distinct reload objects (work += 100 and
...
Write-Host '==> building free-test host (-hot-reload)'
& $odin build $here -out:(Join-Path $work 'free_test.exe') -debug -hot-reload -hot-reload-manifest:$manifest
...
function Build-Variant($add, $dir) {
	...
	& $odin build $src -build-mode:obj -hot-reload -hot-reload-manifest:$manifest -out:(Join-Path $objs 'free_hot.obj')
