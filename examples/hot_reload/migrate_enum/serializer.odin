...
//   https://github.com/Binasaurus-Hex/HotReload  (game/serializer.odin)
// It walks Odin runtime type-info and stores each field by NAME, so `State` (see
// migrate.odin) survives a layout change across a hot reload: add/remove/reorder/
...
// whose NEW layout is described by `dst_ti` (from hot_reload.Type_Change.new). Fields
