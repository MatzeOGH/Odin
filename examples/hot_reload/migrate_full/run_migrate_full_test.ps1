...
& $odin build $exeSrc -out:(Join-Path $work 'migrate_full.exe') -debug -hot-reload -hot-reload-manifest:$manifest
if ($LASTEXITCODE -ne 0) { throw 'exe build failed' }

Write-Host '==> compiling v2 layout to migrate_full_hot.obj (same manifest)'
$objSrc = Stage 'obj_src' 'v2'
& $odin build $objSrc -build-mode:obj -use-single-module -hot-reload -hot-reload-manifest:$manifest -out:(Join-Path $work 'migrate_full_hot.obj')
