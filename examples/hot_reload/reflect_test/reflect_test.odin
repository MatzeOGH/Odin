#+build windows
package hot_reload_reflect_test

// Verifies the reflection (type_info) refresh across a hot reload: after a reload the exe's
...
// `check` is hot-reloadable (not the entry point): after the reload its patched body builds
...
main :: proc() {
	...
	if !hr.apply_dir() { fmt.eprintln("reload failed"); os.exit(1) }
