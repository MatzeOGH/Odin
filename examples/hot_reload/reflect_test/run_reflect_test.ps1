# Automated test: reflection (type_info) refresh across a hot reload.
#
#   powershell -ExecutionPolicy Bypass -File examples\hot_reload\reflect_test\run_reflect_test.ps1
#
# Builds the .exe (-hot-reload, Thing v1: one field, no Extra), simulates an edit that adds
...
Write-Host '==> building reflect-test .exe (-hot-reload, Thing v1)'
& $odin build $here -out:(Join-Path $work 'reflect_test.exe') -hot-reload -hot-reload-manifest:$manifest
...
$edited = $edited -replace '// HR_NEW_FIELD', 'y: i64,'
$edited = $edited -replace '// HR_NEW_TYPE', 'Extra :: struct { p: int, q: string }'
$edited = $edited -replace '// HR_CHECK_NEW', 'ex := fmt.aprint(Extra{p = 5, q = "hi"}); defer delete(ex); g_pass = strings.contains(th, "y =") && strings.contains(ex, "p =") && strings.contains(ex, "q =")'
...
& $odin build $src -build-mode:obj -use-single-module -hot-reload -hot-reload-manifest:$manifest -out:(Join-Path $objdir 'reflect_hot.obj')
